...
class Cli
{
	...
	public function promptInput($prompt_message, $secure = false){
		...
		if(!empty($prompt_message)){
			...
			if($secure and $this->color_supported) $this->write("\033[0;30m\033[40m", 0);
			$input = trim($this->read());
			if($secure and $this->color_supported) $this->write("\033[0m", 0);
		}
		return $input;
	}
	...
	public function getArgs($index = NULL, $default = NULL){
		...
		elseif(array_key_exists($index, $this->args)) return $this->args[$index];
		else return $default;
	}
	...
	public function colorMode($colored = true){
		if(!$this->_isWindows() and $colored) $this->color_supported = true;
		else $this->color_supported = false;
	}
	...
	protected function _parseOptions(){
		if(!empty($this->options) and !empty($this->args)){
			foreach($this->options as $opt => $option){
				$_opt = $opt; $_long_option = $option['long_opt'];
				$flipped_args = array_flip($this->args);
				$opt_key = NULL;
				if(array_key_exists($_opt, $flipped_args)) $opt_key = $flipped_args[$_opt];
				elseif(array_key_exists($_long_option, $flipped_args)) $opt_key = $flipped_args[$_long_option];
				if($opt_key !== NULL){
	...
	protected function _registerOption($option, $long_option = NULL, $description = NULL, $required = false){
		if(!empty($option)){
			...
			if(!empty($long_option)) $this->options[$opt]['long_opt'] = '--' . strval($long_option);
			if(!empty($description)) $this->options[$opt]['description'] = strval($description);
	...
	protected function _registerOptions($options){
		if(!empty($options) and is_array($options)){
			foreach($options as $option){
				if(is_array($option)) call_user_func_array(array($this,'_registerOption'), $option);
			}
		}
		$this->_parseOptions();
	}
	...
	protected function _setHelpNote($help_note){
		if(!empty($help_note)) $this->help_note = strval($help_note);
	}
