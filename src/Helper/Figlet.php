...
class Figlet
{
     ...
     * Automatically tries the Figlet font data directory
    ...
    public function loadFont($filename, $loadgerman = true) {
        ...
        if (!file_exists($filename)) {
            $fontdir = __DIR__ . '/Figlet/fonts/';

            //only for filenames without dir separators
            if (strpos($filename, '/') === false
                && file_exists($fontdir . $filename)
            ) {
                $filename = $fontdir . $filename;
            } else {
                throw new CliException('Figlet font file '
        ...
        if (substr($filename, -3, 3) == '.gz') {
            ...
            if (!function_exists('gzcompress')) {
                throw new CliException('Cannot load gzip compressed fonts since'
        ...
        if (!($fp = fopen($filename, 'rb'))) {
            throw new CliException('Cannot open figlet font file ' . $filename);
        }
        ...
        if (!$compressed) {
            ...
            if (fread($fp, 2) == 'PK') {
                if (!function_exists('zip_open')) {
                    throw new CliException('Cannot load ZIP compressed fonts since'
        ...
        if (substr($header[0], 0, 5) != 'flf2a') {
            throw new CliException('Unknown FIGlet font format.');
        }
